...
function DNS(core){
    ...
    setTimeout(function(){
        self.tld = [self.core.cluster_id, "containership"].join(".");
        ...
        child_process.exec("ifconfig docker0 | grep 'inet addr' | awk -F: '{print $2}' | awk '{print $1}'", function(err, stdout, stderr){
            ...
            self.core.scheduler.follower.container.set_start_arguments("docker", "HostConfig.Dns", function(options){
                if(options.network_mode == "host")
                    return JSON.stringify([self.host_interface]);
                else
                    return JSON.stringify([self.bridge_interface]);
            });

            self.core.scheduler.follower.container.set_start_arguments("docker", "HostConfig.DnsSearch", JSON.stringify([self.tld]))
        });
    }, 2000);
}
    ...
    this.server.persistence.get_configuration(function(err, configuration){
        ...
        return async.parallel({
            create: function(fn){
                async.each(to_create, function(record, fn){
                    if(_.has(applications, record)){
                        if(applications[record].network_mode == "host")
                            var address = self.host_interface;
                        else
                    ...
                    self.server.persistence.update_record([record, self.tld].join("."), { type: "A", address: address }, function(err){
                        if(err)
                            return self.server.persistence.create_record([record, self.tld].join("."), { type: "A", address: address }, fn);
                        else
                            return fn();
                    });
                }, fn);
            ...
            update_leaders: function(fn){
                ...
                self.server.persistence.update_record(["leaders", self.tld].join("."), { type: "A", address: _.compact(address) }, function(err){
                    if(err)
                        return self.server.persistence.create_record(["leaders", self.tld].join("."), { type: "A", address: _.compact(address) }, fn);
                    else
                        return fn();
                });
            },
            ...
            delete: function(fn){
                async.each(to_delete, function(record, fn){
                    return self.server.persistence.delete_record([record, self.tld].join("."), fn);
                }, fn);
            }
        }, fn);
    });
